...
"""
load_timetable_data.py — Management command WITH ELECTIVES
===========================================================
Loads all data including elective courses (CC and IS)
"""
...
class Command(BaseCommand):
    help = 'Load input data including electives'

    def handle(self, *args, **kwargs):
        self.stdout.write('🚀 Starting data load with ELECTIVES...')
        ...
        for uid, name in inst_data:
            T[uid] = Instructor.objects.create(
                uid=uid, 
                name=name,
                max_batches_per_week=4,
                max_lecture_divisions=2
            )
        self.stdout.write(f'✅ {len(T)} instructors')
        ...
        ai = Course.objects.create(
            ...
            course_type='LECTURE',
            is_elective=False)
        ai.instructors.set([T['T001'], T['T004'], T['T018'], T['T021'], T['T022']])
        ...
        dsbda = Course.objects.create(
            ...
            course_type='LECTURE',
            is_elective=False)
        dsbda.instructors.set([T['T002'], T['T005'], T['T023'], T['T008']])
        ...
        wt = Course.objects.create(
            ...
            course_type='LECTURE',
            is_elective=False)
        wt.instructors.set([T['T003'], T['T006'], T['T015']])

        # ✅ ELECTIVE COURSES
        cc = Course.objects.create(
            course_number='CC',
            course_name='Cloud Computing',
            max_numb_students=100,
            course_type='LECTURE',
            is_elective=True)
        cc.instructors.set([T['T010'], T['T009']])  # Chandorkar (G1), Masram (G2)

        is_course = Course.objects.create(
            course_number='IS',
            course_name='Information Security',
            max_numb_students=100,
            course_type='LECTURE',
            is_elective=True)
        is_course.instructors.set([T['T001'], T['T014']])  # Girme (G1), Kapadnis (G2)
        ...
        self.stdout.write('✅ 8 courses created (5 lecture + 3 lab)')
        ...
        # ✅ ELECTIVE SLOTS (2-hour blocks, marked as LECTURE)
        ec = 1
        for day in days:
            for s, e in lab_slots:  # Same timing as labs but LECTURE type
                MeetingTime.objects.create(
                    pid=f'E{ec:03d}', 
                    time=f'{s} - {e}',
                    day=day, 
        ...
        # ✅ ELECTIVE SECTIONS
        # CC Group 1 (TE-I + TE-II)
        Section.objects.create(
            section_id='CC-G1',
            department=dept,
            num_class_in_week=2,  # 1x 2-hour + 1x 1-hour
            division=divisions[0],  # TE-I as representative
            course=cc,
            is_elective=True,
            elective_group='G1',
        )
        
        # CC Group 2 (TE-III + TE-IV)
        Section.objects.create(
            section_id='CC-G2',
            department=dept,
            num_class_in_week=2,
            division=divisions[2],  # TE-III as representative
            course=cc,
            is_elective=True,
            elective_group='G2',
        )
        
        # IS Group 1 (TE-I + TE-II)
        Section.objects.create(
            section_id='IS-G1',
            department=dept,
            num_class_in_week=2,
        ...
        # ── Summary ────────────────────────────────────────────────────────
        self.stdout.write(self.style.SUCCESS('\n✅ DATA LOADED SUCCESSFULLY (WITH ELECTIVES)\n'))
        self.stdout.write('📊 Summary:')
        self.stdout.write(f'   Departments  : {Department.objects.count()}')
        self.stdout.write(f'   Divisions    : {Division.objects.count()}')
        self.stdout.write(f'   Batches      : {Batch.objects.count()}')
        ...
        self.stdout.write('\n📚 ELECTIVE TEACHERS:')
        self.stdout.write('   CC Group 1 (TE-I + TE-II): Prof. A.A. Chandorkar')
        self.stdout.write('   CC Group 2 (TE-III + TE-IV): Prof. B.P. Masram')
        self.stdout.write('   IS Group 1 (TE-I + TE-II): Dr. S.N. Girme')
        self.stdout.write('   IS Group 2 (TE-III + TE-IV): Prof. N.Y. Kapadnis')
